...
function ensurePageBackgroundHost(editorEl) {
  let host = editorEl.querySelector('.rt-page-bg-host');
  if (!host) {
    host = document.createElement('div');
    host.className = 'rt-page-bg-host';
    host.style.position = 'absolute';
    host.style.left = '0';
    host.style.top = '0';
    host.style.right = '0';
    host.style.pointerEvents = 'none';
    editorEl.prepend(host);
  }
  return host;
}
...
function renderPageBackgrounds(editor, pageH_px, marginTop_px, marginBottom_px) {
  ...
  const host = ensurePageBackgroundHost(editorEl);

  // remove ONLY managed children
  ...
  for (let i = 0; i < pageCount; i++) {
    ...
    // width from last page config (A4 / Letter / orientation)
    let pageW_px = 0;
    ...
    // HARD fallback: A4 width (Word / Docs default)
    if (!pageW_px || !isFinite(pageW_px) || pageW_px < 200) {
      pageW_px = mmToPx(210); // A4 portrait width
    }
...
export function runOnce(editor, {
  ...
  try {
    ...
    if (cfg?.size) {
      ...
      prose.style.paddingLeft  = `var(--rt-margin-left)`;
      prose.style.paddingRight = `var(--rt-margin-right)`;
