<?php /* CreateModal */ ?>
<div class="modal fade" id="CreateModal" tabindex="-1" aria-hidden="true" aria-labelledby="CreateCourseLabel">
  <div class="modal-dialog">
    <form method="post" action="<?= BASE_PATH ?>/dashboard?page=courses&action=create" class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="CreateCourseLabel">Create Course</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <input type="hidden" name="csrf" value="<?= $esc($csrf) ?>">

        <div class="mb-3">
          <label class="form-label">Course Code <span class="text-danger">*</span></label>
          <input type="text" name="course_code" class="form-control" maxlength="50" required>
          <div class="form-text">Unique per curriculum.</div>
        </div>

        <div class="mb-3">
          <label class="form-label">Course Name <span class="text-danger">*</span></label>
          <input type="text" name="course_name" class="form-control" maxlength="50" required>
        </div>

        <div class="mb-0">
          <label class="form-label">Link to Curricula (optional)</label>
          <select name="curriculum_ids[]" class="form-select" multiple size="6">
            <?php if (!empty($curricula)): ?>
              <?php foreach ($curricula as $c): ?>
                <option value="<?= (int)$c['curriculum_id'] ?>">
                  <?= $esc($c['curriculum_code']) ?> — <?= $esc($c['curriculum_title']) ?>
                </option>
              <?php endforeach; ?>
            <?php else: ?>
              <option value="" disabled>(No curricula found)</option>
            <?php endif; ?>
          </select>
          <div class="form-text">Hold Ctrl/Cmd (or Shift) to select multiple.</div>
        </div>

        <div class="mb-0 mt-3">
          <label class="form-label">College (optional)</label>
          <select
            id="create-college_id"
            name="department_id"
            class="form-select"
            data-locked="<?= !empty($restrictCollege) ? '1' : '0' ?>"
            data-locked-value="<?= $lockedCollegeId !== null ? (int)$lockedCollegeId : '' ?>">
            <option value="">— None —</option>
            <?php foreach ($colleges as $c): ?>
              <?php $cid = (int)$c['college_id']; ?>
              <option value="<?= $cid ?>" <?= ($lockedCollegeId !== null && $cid === (int)$lockedCollegeId) ? 'selected' : '' ?>>
                <?= $esc($c['short_name']) ?>
              </option>
            <?php endforeach; ?>
          </select>
        </div>

        <?php if (!empty($canAssignProfessors)): ?>
        <div class="mb-0 mt-3">
          <label class="form-label">Assigned Professors</label>
          <select id="create-professor_ids" name="professor_ids[]" class="form-select" multiple size="6">
            <?php foreach ($professors as $prof): ?>
              <option value="<?= $esc($prof['id_no']) ?>">
                <?= $esc($prof['full_name']) ?>
              </option>
            <?php endforeach; ?>
          </select>
          <div class="form-text">Hold Ctrl/Cmd (or Shift) to select multiple professors.</div>
        </div>
        <?php endif; ?>
      </div>

      <div class="modal-footer">
        <button class="btn btn-secondary" type="button" data-bs-dismiss="modal">Cancel</button>
        <button class="btn btn-primary" type="submit">Create</button>
      </div>
    </form>
  </div>
</div>
